...
internal class StepHandler(
    ...
    override fun build(expression: IrCall, data: ProgressionType, scopeOwner: IrSymbol): HeaderInfo? =
        with(context.createIrBuilder(scopeOwner, expression.startOffset, expression.endOffset)) {
            ...
            // This temporary variable for step only needs to be mutable in certain cases (see below).
            val (stepArgVar, stepArgExpression) = createTemporaryVariableIfNecessary(
                stepArg,
                "stepArg",
                isMutable = nestedInfo.direction != ProgressionDirection.INCREASING
            )
